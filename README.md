...
### 🤖 AI agent 主要特性
系统内置了功能强大的 AI agent，打通了自然语言到硬件调试的桥梁，具有以下主要特性：
* **多模型支持与自然语言生成** — 支持 500+ 种 AI 模型，可以直接根据自然语言生成嵌入式驱动、协议解析和应用层代码。
* **模型上下文协议 (MCP) 深度集成** — 深度支持 MCP 协议标准，使 AI agent 能够与外部硬件调试工具、仪器或宿主环境无缝桥接与交互。
* **自定义技能系统 (Skills System)** — 支持为 AI agent 动态扩展专属技能模块，实现与特定硬件外设、专用编译链以及自定义调试流程的灵活联动。
* **任务自动化与工作流** — 能够自动执行编译、烧录、日志监控等硬件调试链条中的各种复杂任务，并支持多步骤调试工作流。
* **多会话编排与 Git 工作区隔离** — 内置 Agent 管理器，提供侧边栏 Chat 交互，支持多个调试会话的并发管理，且会话之间通过 Git 工作区安全隔离。
* **进程级并发与服务通信** — 采用稳定的 CLI 后端作为服务进程，通过 HTTP + SSE 与前端/插件通信，保证 AI 运行时的高效与稳定。
